...
public class TableRendererSorter extends TableMap {
    ...
    private int compareRows(int row1, int row2) {
        ...
        while (comparaison == 0 && columnIterator.hasNext()) {
            ...
            boolean hasSimpleValue = cellRenderer instanceof DefaultTableCellRenderer
                                     || cellRenderer instanceof CheckBoxRenderer;

            TableModel data = getModel();
            Object value1 = data.getValueAt(row1, column);
            Object o1 = getCellValue(row1, columnView, cellRenderer, value1, hasSimpleValue);

            Object value2 = data.getValueAt(row2, column);
            Object o2 = getCellValue(row2, columnView, cellRenderer, value2, hasSimpleValue);

            Class type = data.getColumnClass(column);
            if (!hasSimpleValue && type != Date.class) {
                type = String.class;
            }
    ...
    // Add a mouse listener to the TableInfo to trigger a table sort
    ...
    public void addMouseListenerToHeaderInTable(JTable tbl) {
        ...
        MouseAdapter headerMouseListener = new SorterMouseAdapter(this, tbl);
    ...
    private static class SorterMouseAdapter extends MouseAdapter {
        ...
        private SorterMouseAdapter(TableRendererSorter sorter, JTable table) {
            this.sorter = sorter;
            this.table = table;
        }
        ...
        public void mousePressed(MouseEvent e) {
            int columnIndexToResize = getColumnIndexToResize(e, table.getColumnModel());

            if (columnIndexToResize == -1) {
                sorter.mousePressedOnHeader(e);
            }
        }


        private static int getColumnIndexToResize(MouseEvent event, TableColumnModel columnModel) {
            int x = event.getX();
